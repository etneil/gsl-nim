...
type
  ...
  gsl_siman_Efunc_t* = proc (xp: pointer): cdouble {.cdecl.}
  gsl_siman_step_t* = proc (r: ptr gsl_rng; xp: pointer; step_size: cdouble) {.cdecl.}
  gsl_siman_metric_t* = proc (xp: pointer; yp: pointer): cdouble {.cdecl.}
  gsl_siman_print_t* = proc (xp: pointer) {.cdecl.}
  gsl_siman_copy_t* = proc (source: pointer; dest: pointer) {.cdecl.}
  gsl_siman_copy_construct_t* = proc (xp: pointer): pointer {.cdecl.}
  gsl_siman_destroy_t* = proc (xp: pointer) {.cdecl.}
  gsl_siman_params_t* {.bycopy.} = object
    n_tries*: cint
    iters_fixed_T*: cint
    step_size*: cdouble
    k*: cdouble
...
proc gsl_siman_solve*(r: ptr gsl_rng; x0_p: pointer; Ef: gsl_siman_Efunc_t; take_step: gsl_siman_step_t; distance: gsl_siman_metric_t; print_position: gsl_siman_print_t; copyfunc: gsl_siman_copy_t; copy_constructor: gsl_siman_copy_construct_t; destructor: gsl_siman_destroy_t; element_size: csize_t; params: gsl_siman_params_t) {.cdecl, importc: "gsl_siman_solve", dynlib: libgsl.}
proc gsl_siman_solve_many*(r: ptr gsl_rng; x0_p: pointer; Ef: gsl_siman_Efunc_t;  take_step: gsl_siman_step_t;  distance: gsl_siman_metric_t;  print_position: gsl_siman_print_t;  element_size: csize_t; params: gsl_siman_params_t) {. cdecl, importc: "gsl_siman_solve_many", dynlib: libgsl.}
